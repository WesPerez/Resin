...
管理入口：**https://weesai.com/schedule/projects/resin/**。页面展示任务用途、计划、最近结果、
...
2. 把本提交的 `operations/` 发布到 `/opt/resin-operations/operations/`，以 root 持有。
3. 在受保护的 `/etc/server-scheduled-tasks/schedules.toml` 的 `[projects]` 中登记
   `resin = "/opt/resin-operations"`。本项目计划条目带 `project = "resin"` 和相对来源。
...
共享订阅质量服务隔离了 `/etc/server-scheduled-tasks`，因此只通过 `LoadCredential` 读取
`proxy-subscription-settings.json` 这一份非秘密参数。首次部署创建空对象 `{}`（0600）使用原默认值；
以后保留 Web 写入的参数。不得取消整个目录隔离来让单个参数文件可读。

客户端检查将“通用出口和订阅通过，但暂无合格专用出口”显示为等待资格验证（退出码 75）。
完整验收字段 `ok` 仍为 false，专用出口仍按原标准关闭；页面持续提示缺少合格出口。
实际通用出口、订阅或已合格专用出口探测失败仍是执行失败，不以延后掩盖。
