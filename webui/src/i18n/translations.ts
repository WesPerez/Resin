...
Note: Once enabled, requests without authentication information are rejected instead of being treated as Default platform requests.`,
  ...
  "出口自动恢复": "Automatic egress recovery",
  "自动恢复失败的出口": "Recover failed egress automatically",
  "每分钟最多恢复账号数（每平台）": "Maximum recovered accounts per minute, per platform",
  "平台暂停恢复时长（秒）": "Platform recovery pause (seconds)",
  "在请求失败时直接恢复出口，已有连接继续完成。各平台分别限流；无可用替代时保留当前出口。": "Recover directly when requests fail, allowing existing connections to finish. Each platform has its own limits; the current egress is retained if no alternative is available.",
  "单账号至少间隔 30 秒；15 分钟内恢复 3 次后暂停 1 小时。客户端取消不会触发恢复。": "Each account waits at least 30 seconds between recoveries, and pauses for one hour after three recoveries in 15 minutes. Client cancellation does not trigger recovery.",
  "本次启动以来：恢复 {{rotated}} 次，限流 {{limited}} 次，无替代 {{unavailable}} 次，暂停 {{platforms}} 个平台。": "Since startup: {{rotated}} recovered, {{limited}} limited, {{unavailable}} without alternatives; {{platforms}} platforms paused.",
  "最近处理时间：{{time}}": "Last processed: {{time}}",
  "本次启动后尚无恢复事件": "No recovery events since startup",
  "正在读取恢复状态": "Loading recovery status",
  "恢复状态暂不可用": "Recovery status is temporarily unavailable",
  "已恢复出口": "Egress recovered",
  "等待恢复冷却结束": "Waiting for recovery cooldown",
  "自动恢复已关闭": "Automatic recovery is disabled",
  "暂无可用替代出口，保持原连接": "No alternative egress; current connections retained",
  "旧请求已失效，保持当前出口": "Stale report; current egress retained",
  "恢复失败，请查看请求日志": "Recovery failed; check request logs",
  "{{field}} 必须在 {{min}} 到 {{max}} 之间": "{{field}} must be between {{min}} and {{max}}",
};
